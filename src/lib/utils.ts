...
export function quitarAcentosYMinusculasYEspacios(
...
export function capitalizeOnlyFirstLetterDelete_(
  ...
  return (
    textWithSpaces.charAt(0).toUpperCase() +
    textWithSpaces.slice(1).toLowerCase()
  );
}
...
export function formatVehiculosToBitacora(
  arr: Vehiculo[],
): Vehiculo_bitacora[] {
  return arr.map((item) => ({
    tipo: item.tipo ?? "",
...
export function errorMsj(data: any, title = "Error", type = "warning") {
  if (Array.isArray(data?.response?.data?.json?.msg)) {
    return {
      title: title,
      text:
        data?.response?.data?.json?.label +
        " " +
        data.response.data.json.msg.join(", "),
      type,
    };
  }
  ...
  if (data.hasOwnProperty("json")) {
    const errores = [];
    if (
      data.json.hasOwnProperty("error") &&
      typeof data.json.error === "string"
    ) {
      ...
      for (const err in data.json) {
        ...
        if (data.json[err].hasOwnProperty("label")) {
          errores.push(
            data.json[err].label + ": " + data.json[err].msg[0] + " ",
          );
        } else {
          ...
          if (typeof subData === "object" && subData !== null) {
            for (const subKey in subData) {
              ...
              if (
                subItem &&
                subItem.hasOwnProperty("label") &&
                subItem.hasOwnProperty("msg")
              ) {
  ...
  if (
    ...
    for (const err in dataInner.json) {
      if (dataInner.json[err].hasOwnProperty("label")) {
        errores.push(
          dataInner.json[err].label + ": " + dataInner.json[err].msg[0] + " ",
        );
      } else {
          ...
          for (const subKey in subData) {
            if (subKey !== "group") {
              ...
              for (const subSubKey in subItem) {
                if (
                  typeof subItem[subSubKey] === "object" &&
                  subItem[subSubKey] !== null
                ) {
                  const subSubItem = subItem[subSubKey];
                  if (
                    subSubItem.hasOwnProperty("label") &&
                    subSubItem.hasOwnProperty("msg")
                  ) {
                    errores.push(
                      subSubItem.label + ": " + subSubItem.msg[0] + " ",
                    );
                  }
                }
              }
    ...
    return [
      ...new Set(
        catalogo
          .filter(
            (item) =>
              item["Tipo de vehículo"] === tipo && item["Marca"] === marca,
          )
...
export function calcularTiempoDesdeIncidencia(
...
export const prefijoToCountry: Record<string, string> = {
  "1": "US",
  "52": "MX",
  "54": "AR",
  "55": "BR",
