...
export function mapNotaGrid(raw: any, base: any) {
  ...
  return {
    ...
    detailsList: [
      { icon: <User className="h-3 w-3" />, label: "CREADO POR", value: `Creado por: ${raw?.created_by_name || "-"}` },
      { icon: <CalendarDays className="h-3 w-3" />, label: "APERTURA", value: `Apertura: ${raw?.note_open_date || "-"}` },
      { icon: <MessageSquare className="h-3 w-3" />, label: "COMENTARIOS", value: comments.length > 0 ? `Comentarios: ${comments[0]}` : "-" },
    ],
    modalDetailsList: [
      ...
      { icon: <FileText className="h-3 w-3" />, label: "Archivos", value: raw?.note_file?.map((f: any) => f.file_name).join(", ") || "Sin archivos" },
    ],
    rawData: raw,
    vehiculos: null,
    equipos: null,
