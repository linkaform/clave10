export function mapFallaList(raw: any, base: any) {
  ...
  return {
    ...
    badgesList: [
      {
        customClass: `px-4 py-1 text-xs font-bold capitalize rounded-xl whitespace-nowrap ${estatusClass}`,
        label: raw?.falla_estatus || "Sin estatus",
      },
      {
        customClass:
          "bg-orange-100 hover:bg-orange-100 px-4 py-1 text-xs font-bold text-orange-600 rounded-xl border-0 shadow-none whitespace-nowrap",
        label: raw?.falla_objeto_afectado || "Sin falla",
      },
      {
        customClass:
          "bg-indigo-100 hover:bg-indigo-100 px-4 py-1 text-xs font-bold text-indigo-600 rounded-xl border-0 shadow-none",
        label: raw?.folio || "",
      },
    ],
    detailsList: [
      { icon: null, label: "CASETA", value: raw?.falla_caseta || "-" },
      { icon: null, label: "FALLA", value: raw?.falla_objeto_afectado || "-" },
