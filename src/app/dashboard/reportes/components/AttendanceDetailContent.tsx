...
interface AttendanceDetailContentProps {
    ...
    selectedDay: number;
    ubicacion: string;
    onClose?: () => void; // Optional if used inside a non-modal context or handled parentally
    className?: string; // To allow styling from parent
}

const AttendanceDetailContent: React.FC<AttendanceDetailContentProps> = ({
    names,
    userIds,
    selectedDay,
    ubicacion,
    ...
    const { attendanceDetail, isLoadingAttendanceDetail, errorAttendanceDetail } = useAttendanceDetail({
        enabled: true, // Always enabled as parent controls visibility
        userIds,
        ...
        <div className="flex items-center gap-4 mb-4">
            ...
            <div>
                ...
                <div className="text-xs text-gray-500 flex items-center gap-1 flex-wrap">
                    ...
                    } • {attendanceDetail?.guardia_generales?.incidente_location || ubicacion}
                </div>
                <div className="text-xs text-gray-700 mt-1">
                    Estado laboral: <span className="font-semibold text-green-600">{attendanceDetail?.actual_guard_status || (isLoadingAttendanceDetail ? "Cargando..." : "Activo")}</span>
                </div>
            </div>
        </div>
    );

    if (isLoadingAttendanceDetail) {
        return (
            <div className={`p-4 bg-white rounded-lg h-full ${className}`}>
                <HeaderContent />
                <div className="flex items-center justify-center h-64">
                    <span className="text-gray-500">Cargando detalles...</span>
                </div>
            </div>
        );
    }

    if (errorAttendanceDetail) {
        return (
            <div className={`p-4 bg-white rounded-lg h-full ${className}`}>
                <HeaderContent />
                <div className="flex items-center justify-center h-64">
                    <span className="text-red-500">Error al cargar detalles.</span>
                </div>
            </div>
        );
    }

    // Empty state logic from original modal
    if (Object.keys(attendanceDetail?.guardia_generales || {}).length === 0 && !isLoadingAttendanceDetail) {
        return (
            <div className={`p-4 bg-white rounded-lg h-full ${className}`}>
                <HeaderContent />
                ...
                {onClose && (
                    <Button variant="outline" className="w-full mt-4" onClick={onClose} type="button">Cerrar</Button>
                )}
            </div>
        )
    }
    ...
    return (
        <div className={`p-4 bg-white rounded-lg flex flex-col h-full ${className}`}>
            <HeaderContent />
            ...
            {/* Tabs */}
            <Tabs defaultValue="generales" className="w-full mt-4 flex-1 flex flex-col min-h-0">
                ...
                <TabsContent value="generales" className="flex-1 min-h-0 overflow-hidden mt-2">
                ...
                <TabsContent value="actividades" className="flex-1 min-h-0 relative mt-2">
            ...
            {onClose && (
                <Button variant="outline" className="w-full mt-4" onClick={onClose} type="button">
                    Cerrar
                </Button>
            )}
...
export default AttendanceDetailContent;
