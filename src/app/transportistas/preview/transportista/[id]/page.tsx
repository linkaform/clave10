...
import {
  FileText, Download, Share2, Send, QrCode, Copy, Check, X,
  Loader2, MapPin, Building2, User, Truck, Package, Camera, Sparkles, Plus,
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { useQuery, useMutation } from "@tanstack/react-query";
import { getPassTransportista, generateSubmitTokenTransportista, updateInformationTransportista } from "@/services/endpoints";
...
function InfoRow({ label, value }: { label: string; value?: string | null }) {
  return (
    <div className="flex flex-col gap-0.5">
      ...
      <span className={value ? "text-xs font-semibold text-gray-800 leading-snug break-all" : "text-xs text-blue-300 italic"}>
        {value ?? "—"}
      </span>
    </div>
  );
...
function FormField({ label, value, onChange, placeholder, type = "text", ia, disabled }: {
  label: string;
  value: string;
    ...
    <div>
      ...
      <input
        ...
        className={cn(
          ...
          disabled ? "border-gray-100 bg-gray-50/50 text-gray-500 cursor-not-allowed" : "border-gray-200 bg-gray-50"
        )}
      />
    </div>
  );
...
interface PhotoValue { file_name: string; file_url: string }

function PhotoField({ label, required, ia, value, uploading, onChange, disabled }: {
  label: string;
  required?: boolean;
  ia?: boolean;
  ...
  const handlePreview = () => {
    ...
    else { setLightbox(true); document.body.style.overflow = "hidden"; }
  };
  ...
  return (
    <div>
      ...
      <input ref={ref} type="file" accept="image/*,application/pdf" className="hidden"
        onChange={(e) => { const f = e.target.files?.[0]; if (f && onChange) onChange(f); e.target.value = ""; }} />
      {value?.file_url ? (
        <div className="flex items-center gap-2 h-10 px-2 rounded-xl border border-blue-200 bg-blue-50/50">
          <button type="button" onClick={handlePreview} className="shrink-0">
            ...
            <img src={value.file_url} alt="" className="h-7 w-7 rounded-lg object-cover border border-blue-100 hover:opacity-80 transition-opacity" />
          </button>
          <button type="button" onClick={handlePreview} className="text-xs text-blue-700 font-medium truncate flex-1 text-left hover:underline">
            {value.file_name}
          </button>
          {!disabled && (
            <button type="button" onClick={() => onChange?.(null as any)}
        ...
        <button type="button" onClick={() => ref.current?.click()} disabled={uploading || disabled}
...
export default function PaseEntradaPreviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const emptyPhoto = (): PhotoState => ({ file_name: "", file_url: "", uploading: false });
  ...
  const uploadPhoto = async (key: keyof typeof photos, file: File | null) => {
    if (!file) { setPhotos((p) => ({ ...p, [key]: emptyPhoto() })); return; }
    setPhotos((p) => ({ ...p, [key]: { ...p[key], uploading: true } }));
    try {
      const renamed = new File([file], reemplazarGuionMinuscula(`${key}_transportista ${file.name}`), { type: file.type });
      const result = await uploadImageMutation.mutateAsync({ img: renamed });
      setPhotos((p) => ({ ...p, [key]: { file_name: result?.file_name ?? file.name, file_url: result?.file_url ?? "", uploading: false } }));
    } catch {
  ...
  const { mutate: guardarInfo, isPending: guardando } = useMutation({
    mutationFn: (payload: unknown) => updateInformationTransportista(payload, accountId),
    onSuccess: () => {
  ...
  const addContenedor = () =>
    setContenedores((p) => [...p, { id: crypto.randomUUID(), numero: "", sello: "", tipo: "" }]);
  const removeContenedor = (id: string) =>
    setContenedores((p) => p.filter((c) => c.id !== id));
  const setContenedorField = (id: string, field: "numero" | "sello" | "tipo", value: string) =>
    setContenedores((p) => p.map((c) => c.id === id ? { ...c, [field]: value } : c));
  ...
  useEffect(() => {
    ...
    if (d.conductor_nombre)           { updates.conductorNombre     = d.conductor_nombre;           locked.conductorNombre = true; }
    if (d.conductor_no_licencia)      { updates.conductorLicencia   = d.conductor_no_licencia;      locked.conductorLicencia = true; }
    if (d.conductor_lugar_expedicion) { updates.conductorExpedicion = d.conductor_lugar_expedicion; locked.conductorExpedicion = true; }
    if (d.conductor_vigencia)         { updates.conductorVigencia   = d.conductor_vigencia;         locked.conductorVigencia = true; }

    if (d.ayudante_nombre)            { updates.ayudanteNombre     = d.ayudante_nombre;             locked.ayudanteNombre = true; }
    if (d.ayudante_no_licencia)       { updates.ayudanteLicencia   = d.ayudante_no_licencia;        locked.ayudanteLicencia = true; }
    if (d.ayudante_lugar_expedicion)  { updates.ayudanteExpedicion = d.ayudante_lugar_expedicion;   locked.ayudanteExpedicion = true; }
    if (d.ayudante_vigencia)          { updates.ayudanteVigencia   = d.ayudante_vigencia;           locked.ayudanteVigencia = true; }

    if (d.vehiculo_linea)         { updates.vehiculoLinea     = d.vehiculo_linea;          locked.vehiculoLinea = true; }
    if (d.vehiculo_tipo_unidad)   { updates.vehiculoTipo      = d.vehiculo_tipo_unidad;    locked.vehiculoTipo = true; }
    if (d.vehiculo_marca)         { updates.vehiculoMarca     = d.vehiculo_marca;          locked.vehiculoMarca = true; }
    if (d.vehiculo_modelo)        { updates.vehiculoModelo    = d.vehiculo_modelo;         locked.vehiculoModelo = true; }
    if (d.vehiculo_year)          { updates.vehiculoAño       = d.vehiculo_year;           locked.vehiculoAño = true; }
    if (d.vehiculo_placas)        { updates.vehiculoPlacas    = d.vehiculo_placas;         locked.vehiculoPlacas = true; }
    if (d.vehiculo_no_economico)  { updates.vehiculoEconomico = d.vehiculo_no_economico;   locked.vehiculoEconomico = true; }
    if (d.vehiculo_niv)           { updates.vehiculoNIV       = d.vehiculo_niv;            locked.vehiculoNIV = true; }

    if (Object.keys(updates).length) setForm((p) => ({ ...p, ...updates }));
    setLockedFields(locked);

    const photoUpdates: Partial<typeof photos> = {};
    if (d.conductor_foto_licencia?.[0])      { photoUpdates.conductor    = { ...d.conductor_foto_licencia[0],      uploading: false }; lockedPh.conductor = true; }
    if (d.ayudante_foto_licencia?.[0])       { photoUpdates.ayudante     = { ...d.ayudante_foto_licencia[0],       uploading: false }; lockedPh.ayudante = true; }
    if (d.vehiculo_tarjeta_circulacion?.[0]) { photoUpdates.vehiculo     = { ...d.vehiculo_tarjeta_circulacion[0], uploading: false }; lockedPh.vehiculo = true; }
    if (d.foto_contenedores?.[0])            { photoUpdates.contenedores = { ...d.foto_contenedores[0],            uploading: false }; lockedPh.contenedores = true; }
    if (Object.keys(photoUpdates).length) setPhotos((p) => ({ ...p, ...photoUpdates }));
    setLockedPhotos(lockedPh);

    if (d.contenedores?.length > 0) {
      setContenedores(
        (d.contenedores as Array<{ numero?: string; sello?: string; tipo?: string }>).map((c) => ({
  ...
  const documentos: { tipo?: string; no_doc?: string; file_name: string; file_url: string }[] =
  ...
  const allFieldsLocked =
    ...
    !!lockedPhotos.conductor && !!lockedPhotos.ayudante &&
    !!lockedPhotos.vehiculo  && !!lockedPhotos.contenedores &&
    lockedContenedores;
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
                        ...
                        <div className="flex flex-col min-w-0">
                          {(doc.tipo || doc.no_doc) && (
                            <span className="text-[10px] text-blue-500 font-medium leading-tight">
                              {[doc.tipo, doc.no_doc].filter(Boolean).join(" · ")}
                            </span>
                          )}
              ...
              <div className="w-full space-y-1">
                ...
                {[
                  ...
                  <div key={label} className="flex items-baseline justify-between gap-2 py-1 border-b border-gray-50 last:border-0">
                    ...
                    <span className={cn("text-[11px] font-semibold text-right min-w-0 break-words", value ? "text-gray-700" : "text-gray-300 italic")}>
                      {value ?? "—"}
                    </span>
                  </div>
                ))}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <FormField label="Nombre" value={form.conductorNombre} onChange={(v) => setField("conductorNombre", v)} placeholder="Extraído de licencia..." ia disabled={lockedFields.conductorNombre} />
                <FormField label="No. de licencia" value={form.conductorLicencia} onChange={(v) => setField("conductorLicencia", v)} placeholder="Extraído de licencia..." ia disabled={lockedFields.conductorLicencia} />
                <FormField label="Lugar de expedición" value={form.conductorExpedicion} onChange={(v) => setField("conductorExpedicion", v)} placeholder="Extraído de licencia..." ia disabled={lockedFields.conductorExpedicion} />
                <FormField label="Vigencia" type="date" value={form.conductorVigencia} onChange={(v) => setField("conductorVigencia", v)} ia disabled={lockedFields.conductorVigencia} />
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <p className="font-semibold text-gray-800 text-sm">Ayudante / Conductor 2</p>
              </div>
              <PhotoField label="Foto de licencia"
              ...
              <div className="grid grid-cols-2 gap-3">
                <FormField label="Nombre" value={form.ayudanteNombre} onChange={(v) => setField("ayudanteNombre", v)} placeholder="Extraído de licencia..." ia disabled={lockedFields.ayudanteNombre} />
                <FormField label="No. de licencia" value={form.ayudanteLicencia} onChange={(v) => setField("ayudanteLicencia", v)} placeholder="Extraído de licencia..." ia disabled={lockedFields.ayudanteLicencia} />
                <FormField label="Lugar de expedición" value={form.ayudanteExpedicion} onChange={(v) => setField("ayudanteExpedicion", v)} placeholder="Extraído de licencia..." ia disabled={lockedFields.ayudanteExpedicion} />
                <FormField label="Vigencia" type="date" value={form.ayudanteVigencia} onChange={(v) => setField("ayudanteVigencia", v)} ia disabled={lockedFields.ayudanteVigencia} />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <FormField label="Línea" value={form.vehiculoLinea} onChange={(v) => setField("vehiculoLinea", v)} placeholder="Extraído de tarjeta..." ia disabled={lockedFields.vehiculoLinea} />
              <FormField label="Tipo de unidad" value={form.vehiculoTipo} onChange={(v) => setField("vehiculoTipo", v)} placeholder="Extraído de tarjeta..." ia disabled={lockedFields.vehiculoTipo} />
              <FormField label="Marca" value={form.vehiculoMarca} onChange={(v) => setField("vehiculoMarca", v)} placeholder="Extraído de tarjeta..." ia disabled={lockedFields.vehiculoMarca} />
              <FormField label="Modelo" value={form.vehiculoModelo} onChange={(v) => setField("vehiculoModelo", v)} placeholder="Extraído de tarjeta..." ia disabled={lockedFields.vehiculoModelo} />
              <FormField label="Año" value={form.vehiculoAño} onChange={(v) => setField("vehiculoAño", v)} placeholder="Extraído de tarjeta..." ia disabled={lockedFields.vehiculoAño} />
              <FormField label="Placas" value={form.vehiculoPlacas} onChange={(v) => setField("vehiculoPlacas", v)} placeholder="Extraído de tarjeta..." ia disabled={lockedFields.vehiculoPlacas} />
              <FormField label="No. económico" value={form.vehiculoEconomico} onChange={(v) => setField("vehiculoEconomico", v)} placeholder="Extraído de tarjeta..." ia disabled={lockedFields.vehiculoEconomico} />
              <FormField label="NIV" value={form.vehiculoNIV} onChange={(v) => setField("vehiculoNIV", v)} placeholder="Extraído de tarjeta..." ia disabled={lockedFields.vehiculoNIV} />
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <p className="font-semibold text-gray-800 text-sm">Contenedores</p>
            ...
            {/* Foto general */}
            <PhotoField label="Foto de contenedores"
            ...
            <div className="space-y-2">
              {contenedores.map((c, idx) => (
                <div key={c.id} className="flex items-start gap-2 bg-gray-50 rounded-xl p-3 group">
                  <span className="text-[10px] font-bold text-gray-300 pt-2.5 w-5 shrink-0 text-center">
                    {idx + 1}
                  </span>
                  <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <FormField label="No. contenedor" value={c.numero} onChange={(v) => setContenedorField(c.id, "numero", v)} placeholder="Ej. CRLU2332440" ia disabled={lockedContenedores} />
                    <FormField label="Sello" value={c.sello} onChange={(v) => setContenedorField(c.id, "sello", v)} placeholder="Ej. SL-1234" ia disabled={lockedContenedores} />
                    <FormField label="Tipo" value={c.tipo} onChange={(v) => setContenedorField(c.id, "tipo", v)} placeholder="Ej. 40HC" ia disabled={lockedContenedores} />
                  </div>
            ...
            <Button
              ...
              onClick={() => {
                const payload = {
                  record_id: d._id,
                  conductor: {
                    foto: photos.conductor.file_url ? { file_name: photos.conductor.file_name, file_url: photos.conductor.file_url } : null,
                  ...
                  ayudante: {
                    foto: photos.ayudante.file_url ? { file_name: photos.ayudante.file_name, file_url: photos.ayudante.file_url } : null,
                  ...
                  vehiculo: {
                    foto: photos.vehiculo.file_url ? { file_name: photos.vehiculo.file_name, file_url: photos.vehiculo.file_url } : null,
                  ...
                  foto_contenedores: photos.contenedores.file_url
                    ? { file_name: photos.contenedores.file_name, file_url: photos.contenedores.file_url }
                    : null,
              ...
              <div className="rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-500 p-4 space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  <div className="bg-white/10 border border-white/15 rounded-xl px-3 py-2 flex items-center gap-2">
                    <p className="text-[11px] text-white/90 font-mono break-all leading-relaxed flex-1">{submitUrl}</p>
                    <button type="button" onClick={handleCopyUrl} className="shrink-0 text-white/50 hover:text-white transition-colors">
                      {copiedUrl ? <Check className="w-3.5 h-3.5 text-green-300" /> : <Copy className="w-3.5 h-3.5" />}
                    </button>
                  </div>
                </div>
                <div className="space-y-1">
                  ...
                  <div className="bg-white rounded-xl px-4 py-3 flex items-center justify-center gap-2 shadow-sm overflow-hidden">
                    <p className="text-xs text-indigo-600 font-mono font-bold tracking-[0.08em] break-all text-center leading-relaxed flex-1">{submitToken}</p>
                    <button type="button" onClick={handleCopyToken} className="shrink-0 text-indigo-300 hover:text-indigo-500 transition-colors">
      ...
      {confirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          <div className="absolute inset-0 bg-black/40" onClick={() => !guardando && setConfirmOpen(false)} />
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-gray-800 text-sm">Confirmar información</p>
                <p className="text-xs text-gray-400 mt-0.5">¿Deseas guardar la información complementaria del pase?</p>
              </div>
            </div>
            <p className="text-xs text-gray-500 bg-gray-50 rounded-xl px-4 py-3 leading-relaxed">
              Esta acción actualizará los datos del conductor, vehículo, ayudante y contenedores asociados al pase.
            </p>
            <div className="grid grid-cols-2 gap-2 pt-1">
              <button type="button" onClick={() => setConfirmOpen(false)} disabled={guardando}
