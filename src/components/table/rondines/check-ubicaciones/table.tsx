...
const EMPTY_CHECK_UBICACIONES: CheckUbicacion[] = [];
...
const CheckUbicacionesTable: React.FC<CheckUbicacionesTableProps> = ({
  ...
  const memoizedData = useMemo(() => EMPTY_CHECK_UBICACIONES, []);
  ...
  const photoListRecords: ListRecord[] = useMemo(() => {
    return memoizedData.map((item) => formatListRecord(item, "incidencia"));
  }, [memoizedData]);
  
  const photoRecords: PhotoRecord[] = useMemo(() => {
    return memoizedData.map((item) => formatPhotoRecord(item, "incidencia"));
  }, [memoizedData]);
