...
import { Bitacora_record as SharedBitacoraRecord } from "../bitacoras-columns";
...
interface ListProps {
  ...
  handleSalida: (bitacora: SharedBitacoraRecord) => void;
...
const EquiposTable: React.FC<ListProps> = ({
  data,
  isLoading,
  printPase,
  total,
  ...
  const photoListRecords: ListRecord[] = useMemo(() => {
    return memoizedData.map((item) =>
      formatListRecord(item, "bitacoras_equipos"),
    );
  }, [memoizedData]);

  const photoRecords: PhotoRecord[] = useMemo(() => {
    return memoizedData.map((item) =>
      formatPhotoRecord(item, "bitacoras_equipos"),
    );
  }, [memoizedData]);
  ...
  const renderActions = (record: PhotoRecord | ListRecord) => {
    ...
    return (
      <InAndOutButtons
        bitacora={bitacora as unknown as SharedBitacoraRecord}
