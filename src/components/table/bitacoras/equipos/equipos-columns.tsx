...
const OptionsCell: React.FC<{ row: any }> = ({ row }) => {
  ...
  bitacora.formated_visita = bitacora.visita_a
    .map((item: VisitaA) => item.nombre)
    .join(", ");
  bitacora.formated_comentarios = bitacora.comentarios
...
export const equiposColumns: ColumnDef<Bitacora_record>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div
          className={`capitalize font-semibold ${isAbierto ? "text-green-600" : "text-red-600"}`}>
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <div className="capitalize">{eq?.tipo_equipo}</div>;
    },
    enableSorting: true,
  },
  {
    ...
    cell: ({ row }) => {
      const eq = row.original.equipos[0];
      return <div className="capitalize">{eq?.nombre_articulo}</div>;
    },
    enableSorting: true,
  },
  {
    ...
    cell: ({ row }) => {
      const eq = row.original.equipos[0];
      return <div className="capitalize">{eq?.marca_articulo}</div>;
    },
    enableSorting: true,
  },

  {
    ...
    cell: ({ row }) => {
      const eq = row.original.equipos[0];
      return <div className="capitalize">{eq?.modelo_articulo}</div>;
    },
    enableSorting: true,
  },

  {
    ...
    cell: ({ row }) => {
      const eq = row.original.equipos[0];
      return <div className="capitalize">{eq?.numero_serie}</div>;
    },
    enableSorting: true,
  },
  {
    ...
    cell: ({ row }) => {
      const eq = row.original.equipos[0];
      return <div className="capitalize">{eq?.color_articulo}</div>;
    },
    enableSorting: true,
  },
];
