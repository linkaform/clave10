  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {fecha && (  
          <p className="text-[10px] text-slate-500 mt-0.5">{fecha}</p>
        )}
      </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {evidencias.map((ev: any, i: number) => (
              <div 
                key={i} 
                className="relative w-full rounded-xl overflow-hidden border border-slate-100 bg-slate-50 aspect-square" 
                style={{ minHeight: '120px' }} 
              >
                <Image
                  src={typeof ev === 'string' ? ev : ev.file_url} 
                  alt={`Evidencia ${i + 1}`}
                  fill
...
function ComentarioListItem({ item, onClick }: { item: any; onClick: () => void }) {
  ...
  return (
    <div
      onClick={onClick}
      className="flex items-start gap-3 py-2 border-b border-slate-100 last:border-0 cursor-pointer hover:bg-slate-50 rounded-lg px-1 -mx-1 transition-colors"
    >
      <div className="w-10 h-10 rounded-lg shrink-0 bg-slate-100 flex items-center justify-center">
        <FileText className="w-4 h-4 text-slate-400" />
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-sm font-medium text-slate-800 leading-tight line-clamp-2">{texto}</p>
        {fecha && (
          <p className="text-[10px] text-slate-500 mt-0.5">{fecha}</p>
        )}
      </div>
                ...
                {comentariosGenerales.length === 0 ? (
                  ...
                  comentariosGenerales.map((item: any, i: number) => (
                    <ComentarioListItem
                      key={i}
                      item={item}
                      onClick={() => setSelectedComentario(item)}
                    />
                  ))
                )}
              </div>
