...
const AreasList: React.FC<AreasListProps> = ({
  ...
  const onSubmitArea = (data: z.infer<typeof formSchema>) => {
    const isAreaExists = areas.some(
      (area) => area.nombre_area === data.nombre_area,
    );
    if (isAreaExists) {
  ...
  const updatedArea = (index: number, value: string, fieldName: string) => {
    ...
    if (
      ...
      areas.some((area) => area.nombre_area === value)
    ) {
      setAreas((prevState) =>
        prevState.filter((area) => area.nombre_area !== value),
      );
    }
  };
        ...
        return (
          <div key={index} className="rounded mt-2">
            <AreasItem
              ...
              updateArea={(value: string, fieldName: string) =>
                updatedArea(index, value, fieldName)
              }
            />
          </div>
        );
