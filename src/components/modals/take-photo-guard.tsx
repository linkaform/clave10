...
interface TakeModalProps {
	...
	// Se dispara cuando el usuario elige continuar sin foto (cámara sin
	// permiso/no disponible) — le toca al padre decidir qué hacer (ej. abrir
	// el modal de confirmación de todos modos).
	onSkip?: () => void;
  ...
  export const TakePhotoGuard: React.FC<TakeModalProps> = ({
	...
	// No pasar el objeto de error crudo a console.error: en dev, el overlay
	// de Next.js lo intercepta y lo muestra como si fuera un crash (aunque
	// aquí ya se maneja bien con el estado cameraError).
	console.warn("No se pudo acceder a la cámara:", err instanceof DOMException ? err.message : err);
	setloadingWebcam(false);
	setCameraError(true);
	};
    ...
    <Dialog open={open} onOpenChange={setOpen} modal>
			...
			<div className="flex justify-center items-center h-[350px] overflow-hidden rounded-lg">
				{cameraError ? (
					<div className="flex flex-col items-center gap-2 text-center px-6">
						<CameraOff className="w-10 h-10 text-amber-500" />
						<p className="text-sm font-medium text-gray-700">
							No se pudo acceder a la cámara.
						</p>
						<p className="text-xs text-gray-400">
							Verifica los permisos del navegador, o continúa sin foto.
						</p>
					</div>
				) : (
					<>
		...
		<div className="flex gap-4 mt-auto">
			...
			<Button
			type="submit"
			className={`w-full text-white ${cameraError ? "bg-amber-500 hover:bg-amber-600" : "bg-blue-500 hover:bg-blue-600"}`}
			onClick={cameraError ? handleContinuarSinFoto : takeAndSavePhoto}
			disabled={isLoading || evidencia.length>0 || (loadingWebcam && !cameraError) }>
			 {cameraError ? (
					<>
					<CameraOff /> Continuar sin foto
					</>
				) : loadingWebcam ? (
					<>
