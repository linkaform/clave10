...
import { Loader2, Settings, CalendarClock, Layers } from "lucide-react";
...
export const AddRondinModal: React.FC<AddRondinModalProps> = ({
	...
	function obtenerRecurrencia(values: Record<string, any>): Record<string, any> {
		switch (values.se_repite_cada) {
		  ...
		  case "semana":
			return {
				...
				...(!todas_las_semanas && { en_que_semana_sucede: en_que_semana_sucede }),
				sucede_recurrencia: ['dia_de_la_semana'],
			};
		  case "mes": {
				if (esRepetirCada == false) {
				  return { se_repite_cada: "mes", sucede_recurrencia: ["dia_del_mes"], en_que_mes: en_que_mes, cada_cuantos_meses_se_repite: values.cada_cuantos_meses_se_repite };
				}
				if (esRepetirCada == true) {
				  return { se_repite_cada: "configurable", que_dias_de_la_semana: que_dias_de_la_semana, sucede_recurrencia: ["dia_de_la_semana"], ...(todas_las_semanas ? {} : { en_que_semana_sucede: en_que_semana_sucede }), en_que_mes: en_que_mes };
				}
				return {};
			}
		  case "configurable":
			return { se_repite_cada: 'configurable', cron_conf: values.cron_conf ?? "" };
		  default:
			return {};
		}
	}
	...
	useEffect(() => {
		if (mode === "edit" && rondinData && isSuccess) {
			...
			if (rondinData.fecha_hora_programada) setDate(new Date(rondinData.fecha_hora_programada));
			if (rondinData.areas && catalogAreasRondin) {
				const cat = (rondinData?.areas ?? []).map((area:any ) => ({ name: area ?? "", id: area ?? "" }));
			...
			if (rondinData?.que_dias_de_la_semana && rondinData?.que_dias_de_la_semana.length>0) {
				...
				set_que_dias_de_la_semana(Array.isArray(rondinData.que_dias_de_la_semana) ? rondinData.que_dias_de_la_semana : []);
			}
			if (!("se_repite_cada" in rondinData)) setNoRecurrente(true);
			if (rondinData?.en_que_semana_sucede && rondinData?.en_que_semana_sucede.length>0) {
				...
				if (rondinData.en_que_semana_sucede !== "todas_las_semanas") set_todas_las_semanas(false);
			}
			if (rondinData?.en_que_mes && rondinData?.en_que_mes.length>0) {
				const meses = Array.isArray(rondinData.en_que_mes) ? rondinData.en_que_mes : [rondinData.en_que_mes];
			...
			if (rondinData.cada_cuantas_horas_se_repite) { setMostrarFrecuencia(true); form.setValue("cada_cuantas_horas_se_repite", rondinData.cada_cuantas_horas_se_repite); }
			if (rondinData.cada_cuantos_meses_se_repite) setEsRepetirCada(true);
			else if (rondinData.en_que_mes && rondinData.en_que_mes.length > 0) setEsRepetirCada(false);
			reset({
	...
	useEffect(() => {
		...
		const resizeObserver = new ResizeObserver(() => { updatePosition(); setTimeout(updatePosition, 100); });
		const handleClick = () => { setTimeout(updatePosition, 10); setTimeout(updatePosition, 50); setTimeout(updatePosition, 100); };
		...
		return () => { resizeObserver.disconnect(); container.removeEventListener('click', handleClick); container.removeEventListener('focus', handleClick, true); };
	  }, [areasSeleccionadas]);
	...
	return (
		...
		<DialogContent className="max-w-2xl max-h-[90vh] flex flex-col p-0 gap-0 rounded-3xl overflow-hidden border-none [&>button]:z-50" aria-describedby="" onInteractOutside={(e) => e.preventDefault()}>
			<div className="px-6 pt-6 pb-3 flex-shrink-0 text-center border-b border-gray-100 bg-white">
				<DialogTitle className="text-2xl font-bold text-gray-800">
					{title}
				</DialogTitle>
				<p className="text-sm text-gray-400 mt-1">
					{mode === "edit" ? "Edita los datos del rondín" : "Completa los datos del nuevo rondín"}
				</p>
			</div>
			...
			<div className="flex-1 overflow-y-auto px-6 py-5 bg-gray-50/50">
				...
				<SectionCard icon={<Settings size={15} />} label="Información general">
				<div className="flex flex-col gap-4">
					...
					<FormField
						...
						render={({field}:any) => (
							<FormItem>
								...
								<FormControl>
									<Input placeholder="Texto" className="rounded-xl border-gray-200 bg-gray-50 focus:ring-2 focus:ring-blue-300" {...field} />
								</FormControl>
								<FormMessage />
							</FormItem>
						)}
					...
					<FormField
						...
						render={({ field }:any) => (
							<FormItem>
								<FormLabel className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Fecha y hora programada <span className="text-red-400">*</span></FormLabel>
								<FormControl>
									<DateTimePicker
										showTime={true}
										allowPast={true}
										...
										setDate={(d) => {
											field.onChange(d ? format(d, "yyyy-MM-dd HH:mm:ss") : "");
											setDate(d ?? "");
										}}
									/>
								</FormControl>
					...
					<div className="flex flex-row gap-3 items-end">
						<FormField
							...
							render={({field}:any) => (
								<FormItem>
									<FormLabel className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Duración estimada <span className="text-red-400">*</span></FormLabel>
									<FormControl>
						...
						<FormField
							...
							render={({ field }:any) => (
								<FormItem className="flex-1">
									<FormLabel className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Tipo de rondín</FormLabel>
									<FormControl>
				...
				<SectionCard icon={<CalendarClock size={15} />} label="Recurrencia">
				<div className="flex flex-col gap-4">
				{noRecurrente && 
					<div className="px-3 py-2 rounded-xl bg-yellow-50 border border-yellow-100 text-sm font-semibold text-yellow-700">Rondín no recurrente</div>
				}
					<FormField
						control={form.control}
						name="se_repite_cada"
					...
					{recurrencia === "diario" && (
						<div className="flex gap-10">
							<div className="mb-2 flex items-center gap-3">
								...
								<Switch checked={mostrarFrecuencia} onCheckedChange={(v) => setMostrarFrecuencia(v)} />
							</div>
							{mostrarFrecuencia && (
							<div className="flex items-center gap-3">
								...
								onChange={(e) => form.setValue("cada_cuantas_horas_se_repite", e.target.value, { shouldValidate: true, shouldDirty: true })}
					...
					<div className="mt-2">
						<div className="flex justify-between">
							...
							<div className="flex items-center gap-2 ml-3">
								<input type="checkbox" onChange={(e:any) => set_todas_las_semanas(e.target.checked)} />
						...
						{["Primer semana del mes","Segunda semana del mes","Tercer semana del mes","Cuarta semana del mes","Quinta semana del mes"].map((semana) => {
							...
							<FormItem key={value} className="flex items-center space-x-3">
								...
								>
									<div className="flex flex-wrap"><div>{semana}</div></div>
								</Button>
								</FormControl>
							</FormItem>
							);
							...
							render={({ field }:any) => (
								<FormItem>
									...
									<Select onValueChange={field.onChange} value={field.value} disabled={!date}>
										...
										<SelectContent>
											{opciones.map((op, i) => (<SelectItem key={i} value={op}>{op}</SelectItem>))}
										</SelectContent>
									</Select>
									</FormControl>
									<FormMessage />
					...
					{recurrencia=="mes" && 
						<div className="flex flex-wrap gap-4">
							...
							className={`px-4 py-2 rounded-md transition-all duration-300 ${esRepetirCada === true ? "bg-blue-600 text-white hover:bg-blue-700 hover:text-white" : "border-2 border-blue-400 bg-white text-blue-600 hover:bg-transparent hover:shadow-[0_3px_6px_rgba(0,0,0,0.2)]"}`}>
							Repetir cada X mes
							</Button>
							<Button type="button" onClick={() => seleccionar(false)}
							className={`px-4 py-2 rounded-md transition-all duration-300 ${esRepetirCada === false ? "bg-blue-600 text-white hover:bg-blue-700 hover:text-white" : "border-2 border-blue-400 bg-white text-blue-600 hover:bg-transparent hover:shadow-[0_3px_6px_rgba(0,0,0,0.2)]"}`}>
							Seleccionar meses
							</Button>
						</div>
					}
					...
					{esRepetirCada === true && (
						...
						render={({ field }) => (
						  <div className="mt-3">
							<label className="text-sm font-medium mb-1 block">Se repetirá cada:</label>
								...
								<div className="mt-2">
									<div className="flex justify-between">
										...
										<div className="flex items-center gap-2 ml-3">
											...
											onChange={(e) => {
												...
												if (checked) {
													set_en_que_mes(["enero","febrero","marzo","abril","mayo","junio","julio","agosto","septiembre","octubre","noviembre","diciembre"]);
												} else {
													set_en_que_mes([]);
												}
											}}
									...
									{mesesDelAño.map((mes) => {
										...
										<FormItem key={mesLower} className="flex items-center space-x-3">
											<FormControl>
											<Button type="button" onClick={() => toggleMes(mesLower)}
												className={`m-2 px-4 py-2 rounded-md transition-all duration-300 ${en_que_mes.includes(mesLower) ? "bg-blue-600 text-white" : "border-2 border-blue-400 bg-white"} hover:bg-transparent hover:shadow-[0_3px_6px_rgba(0,0,0,0.2)]`}>
												<div className="flex flex-wrap">
												<div className={en_que_mes.includes(mesLower) ? "" : "text-blue-600"}>{mes}</div>
												</div>
											</Button>
											</FormControl>
										</FormItem>
					...
					<FormField
						...
						<FormItem>
							...
							<FormControl>
							<Input placeholder="* * * * *" className="resize-none"
								value={field.value ?? ""}
								onChange={(e) => {
				...
				<SectionCard icon={<Layers size={15} />} label="Áreas">
				<div className="relative multiselect-wrapper" ref={multiselectRef}>
				<div className="text-sm mb-2">Áreas: </div>
				{isLoadingAreas ? (
					<div className="border rounded-lg p-3 bg-gray-50 text-gray-500 text-sm">Cargando áreas...</div>
				) : (
					<Multiselect
						...
						style={{
							...
							searchBox: { position: "relative", opacity: isLoadingAreas ? 0.5 : 1 }
						}}
						/>
				)}
				</div>
			...
			<div className="flex-shrink-0 bg-white border-t border-gray-100 px-6 py-4 flex gap-3">
				<DialogClose asChild>
					<Button className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl h-11 font-semibold" onClick={handleClose}>
					Cancelar
					</Button>
				</DialogClose>
				<Button type="button" onClick={form.handleSubmit(onSubmit)} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white rounded-xl h-11 font-semibold" disabled={isLoading || isLoadingEdit}>
					{isLoading || isLoadingEdit ? (
					<>
					{mode === "edit" ? (
						<span className="flex items-center gap-2"><Loader2 className="animate-spin" size={16} /> Actualizando rondin...</span>
					) : (
						<span className="flex items-center gap-2"><Loader2 className="animate-spin" size={16} /> Creando rondin...</span>
					)}
					</>
