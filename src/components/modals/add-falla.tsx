...
const formSchema = z.object({
  ...
  falla_comentarios: z
    .string()
    .min(1, { message: "Comentario es obligatorio" }),
  falla_documento: z
    .array(
      z.object({
        file_url: z.string(),
  ...
  falla_reporta_nombre: z
    .string()
    .min(1, { message: "El nombre del reportante es obligatorio" }),
...
export const AddFallaModal: React.FC<AddFallaModalProps> = ({
  ...
  const { data: dataAreaEmpleadoApoyo, refetch: refetchAreaEmpleadoApoyo } =
    useCatalogoAreaEmpleadoApoyo(isSuccess);
  const { data: dataFallas, refetch: refetchFallas } = useCatalogoFallas(
    subconcepto,
    isSuccess,
  );
  const { createFallaMutation, isLoading } = useFallas(
    "",
    "",
    "abierto",
    false,
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (date) {
      ...
      const formatData = {
        ...
        falla_responsable_solucionar_nombre:
          values.falla_responsable_solucionar_nombre || "",
      ...
      form.setError("falla_fecha_hora", {
        type: "manual",
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setOpen} modal>
      ...
      <DialogContent
        className="max-w-5xl  overflow-y-auto max-h-[80vh] flex flex-col overflow-hidden"
                    ...
                    <form
                            ...
                            <Select
                              ...
                              options={
                                ubicaciones && ubicaciones.length > 0
                                  ? formatForMultiselect(ubicaciones)
                                  : []
                              }
                        ...
                        render={({ field }: any) => (
                          <FormItem className="w-full">
                            ...
                            <Select
                              ...
                              // selectedValues={afectacionPatrimonialSeleccionada? formatForMultiselect([afectacionPatrimonialSeleccionada?.tipo_afectacion]):[]}
                              options={
                                catalagoFallas && catalagoFallas.length > 0
                                  ? formatForMultiselect(catalagoFallas)
                                  : []
                              }
                              onChange={(selectedOption) => {
                                field.onChange(
                        ...
                        render={({ field }: any) => (
                          <FormItem className="w-full">
                            ...
                            <Select
                              ...
                              // selectedValues={afectacionPatrimonialSeleccionada? formatForMultiselect([afectacionPatrimonialSeleccionada?.tipo_afectacion]):[]}
                              options={
                                catalagoSub && catalagoSub.length > 0
                                  ? formatForMultiselect(catalagoSub)
                                  : []
                              }
                        ...
                        render={({ field }: any) => (
                          <FormItem className="w-full">
                            ...
                            <Select
                              ...
                              placeholder="Reporta"
                              options={
                                dataAreaEmpleadoApoyo &&
                                dataAreaEmpleadoApoyo.length > 0
                                  ? formatForMultiselect(dataAreaEmpleadoApoyo)
                                  : []
                              }
                              onChange={(selectedOption: any) => {
                                field.onChange(
                      ...
                      <FormField
                        ...
                        render={({ field }: any) => (
                          <FormItem className="w-full">
                            <FormLabel>
                              Responsable asignado de solucionar:
                            </FormLabel>
                            <Select
                              className="border border-slate-100"
                              ...
                              // selectedValues={afectacionPatrimonialSeleccionada? formatForMultiselect([afectacionPatrimonialSeleccionada?.tipo_afectacion]):[]}
                              options={
                                dataAreaEmpleado && dataAreaEmpleado.length > 0
                  ...
                  <div className="flex gap-2 mb-4">
                    <div className="w-full flex gap-2">
                      ...
                      <span className="font-bold">
                        {" "}
                        {selectedFalla ? (
                          <div className="text-end">{selectedFalla}</div>
                        ) : (
                          <div className="text-gray-400 text-sm text-end font-normal">
                            No se ha seleccionado una falla.
                          </div>
                        )}
                      </span>
                    </div>
                        ...
                        seguimientos.map((item: any, index: number) => (
                          <tr key={index} className="border-t border-gray-200">
                            <td className="px-4 py-2">
                              {item?.fecha_inicio_seg || "N/A"}
                            </td>
                            <td className="px-4 py-2">
                              {item?.tiempo_transcurrido}
                            </td>
                            <td className="px-4 py-2">
                              {item?.accion_correctiva_incidencia || "N/A"}
                            </td>
                            <td className="px-4 py-2">
                              {item?.incidencia_personas_involucradas || "N/A"}
                            </td>

                            <td className="px-4 py-2 min-w-[150px]">
                              {item?.incidencia_evidencia_solucion?.length >
                              0 ? (
                                <div className="w-full flex justify-center">
                                  <Carousel className="w-16">
                                    <CarouselContent>
                                      {item.incidencia_evidencia_solucion.map(
                                        (a: any, i: number) => (
                                          <CarouselItem key={i}>
                                            <Card>
                            ...
                            <td className="px-4 py-2">
                              {item?.incidencia_documento_solucion?.length >
                              0 ? (
                                <ul className="ms-2">
                                  {item?.incidencia_documento_solucion.map(
                                    (file: any, i: number) => (
                                      <li key={i}>
                                        <a
                                          href={file?.file_url}
