...
export const EntryPassModal2: React.FC<EntryPassModal2Props> = ({
  ...
  const [responseformated, setResponseFormated] = useState<data_correo | null>(
    null,
  );
  ...
  return (
    <Dialog open={isSuccess} onOpenChange={setIsSuccess} modal>
      <DialogContent
        className="max-w-3xl overflow-y-auto max-h-[90vh] flex flex-col  "
        aria-describedby=""
              ...
              <div className="flex justify-between">
                <div className="w-full ">
                  ...
                  {data && data?.walkin_fotografia.length > 0 ? (
                    ...
                    <div>No hay fotografía disponible</div>
                  )}
                </div>
              </div>
              <div className="flex justify-between">
                <div className="w-full ">
                  ...
                  {data && data?.walkin_identificacion.length > 0 ? (
                    ...
                    <div>No hay identificación disponible</div>
                  )}
                </div>
              </div>
            </div>
                    ...
                    <AccordionContent className="mb-0 pb-0">
                      {data?.grupo_equipos.length > 0 ? (
                        <table className="min-w-full table-auto border-separate border-spacing-2">
                          ...
                          <tbody>
                            {data?.grupo_equipos.map(
                              (item: Equipo, index: number) => (
                                <tr key={index}>
                                  ...
                                  <td className="px-4 py-2">
                                    <small>
                                      {capitalizeFirstLetter(item.marca ?? "")}
                                    </small>
                                  </td>
                                  <td className="px-4 py-2">
                                    <small>
                                      {capitalizeFirstLetter(item.modelo ?? "")}
                                    </small>
                                  </td>
                                  <td className="px-4 py-2">
                                    <small>
                                      {capitalizeFirstLetter(item.serie ?? "")}
                                    </small>
                                  </td>
                                  <td className="px-4 py-2">
                                    <small>
                                  ...
                                  <td className="px-4 py-2">
                                    {item.foto_equipo &&
                                    item.foto_equipo.length > 0 ? (
                                      <Image
                                        src={
                                          item.foto_equipo[0].file_url ??
                                          "/nouser.svg"
                                        }
                                        alt="Foto equipo"
                                        width={40}
                    ...
                    <AccordionContent className="mb-0 pb-0">
                      {data?.grupo_vehiculos.length > 0 ? (
                        <table className="min-w-full table-auto border-separate border-spacing-2">
                          ...
                          <tbody>
                            {data?.grupo_vehiculos?.map(
                              (item: Vehiculo, index: number) => (
                                <tr key={index}>
                                  <td className="px-4 py-2">
                                    <small>
                                      {capitalizeFirstLetter(item.tipo ?? "")}
                                  ...
                                  <td className="px-4 py-2">
                                    <small>
                                      {capitalizeFirstLetter(item.placas ?? "")}
                                    </small>
                                  </td>
                                  <td className="px-4 py-2">
                                    <small>
                                  ...
                                  <td className="px-4 py-2">
                                    {item.foto_vehiculo &&
                                    item.foto_vehiculo.length > 0 ? (
                                      <Image
                                        src={
                                          item.foto_vehiculo[0].file_url ??
                                          "/nouser.svg"
                                        }
                                        alt="Foto vehículo"
                                        width={40}
