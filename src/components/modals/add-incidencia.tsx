...
export const AddIncidenciaModal: React.FC<AddIncidenciaModalProps> = ({
  ...
  const { dataAreas: areas, dataLocations: ubicaciones } =
  ...
  const { data: dataAreaEmpleado } = useCatalogoAreaEmpleado(
    isSuccess,
    location ?? "",
    "Incidencias",
  );
  ...
  const { catIncidencias, isLoadingCatIncidencias } = useCatalogoInciencias(
    isSuccess,
    categoria,
    subCategoria,
  );
  ...
  const [
    ...
    setAfectacionPatrimonialSeleccionada,
  ] = useState<AfectacionPatrimonial | null>(null);
  const [afectacionPatrimonial, setAfectacionPatrimonial] = useState<
    AfectacionPatrimonial[]
  >([]);
  const [openAfectacionPatrimonialModal, setOpenAfectacionPatrimonialModal] =
    useState(false);
  const [editarAfectacionPatrimonial, setEditarAfectacionPatrimonial] =
    useState(false);
  ...
  const seguimientosOrdenados = [...seguimientos].sort((a, b) => {
    return (
      new Date(a.fecha_inicio_seg).getTime() -
      new Date(b.fecha_inicio_seg).getTime()
    );
  });
    ...
    if (catIncidencias) {
      if (search == "") {
        ...
        if (catIncidencias.type == "incidence") {
          const formattedSubIncidentes = catIncidencias.data.map(
            (nombre: string) => ({
              id: nombre,
              nombre,
              icono: "",
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (!date) {
      ...
      const formatData = {
        ...
        //Perosona extraviada
        nombre_completo_persona_extraviada:
          values.nombre_completo_persona_extraviada,
        edad: values.edad,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setOpen} modal>
      <DialogContent
        className="max-w-7xl overflow-y-auto max-h-[80vh] min-h-[80vh]  flex flex-col overflow-hidden"
        ...
        {selectedIncidencia && (
          <>
                      ...
                      <Form {...form}>
                        <form
                                ...
                                <Select
                                  ...
                                  options={
                                    ...
                                    dataAreaEmpleado.length > 0
                                      ? formatForMultiselect(dataAreaEmpleado)
                                      : []
                                  }
                                  value={
                                    formatForMultiselect(dataAreaEmpleado).find(
                            ...
                            render={({ field }: any) => (
                              <FormItem>
                                ...
                                <Select
                                  ...
                                  isClearable
                                  value={
                                    formatForMultiselect(areas).find(
                                      (option) => option.value === field.value,
                                    ) || ""
                                  }
                                  styles={{
                          ...
                          <div className="flex items-center flex-wrap gap-5">
                            <FormLabel>
                              Notificaciones: {`(No/Correo)`}:{" "}
                            </FormLabel>
                            <Switch
                              defaultChecked={false}
                              onCheckedChange={() => {
                                setSelectedNotification(!selectedNotificacion);
                              }}
                              aria-readonly
                            />
                          </div>
                          ...
                          {selectedIncidencia == "Robo de cableado" && (
                            <div className="col-span-2 w-full flex flex-col ">
                              ...
                              <RoboDeCableado
                                control={form.control}></RoboDeCableado>
                            </div>
                          )}
                          {selectedIncidencia == "Robo de vehículo" && (
                            <div className="col-span-2 w-full">
                              <RoboDeVehiculo
                                control={form.control}></RoboDeVehiculo>
                            </div>
                          )}
                          {selectedIncidencia ==
                            "Depósitos y retiros de valores" && (
                            <div className="col-span-1 md:col-span-2">
                              <SeccionDepositos
                                depositos={depositos}
                                setDepositos={setDepositos}></SeccionDepositos>
                            </div>
                          )}
                        </form>
                      </Form>
                      ...
                      <table className="min-w-full table-auto mb-5 border">
                        ...
                        <tbody>
                          ...
                          seguimientosOrdenados.length > 0 ? (
                            seguimientosOrdenados.map(
                              (item: any, index: number) => (
                                <tr
                                  key={index}
                                  className="border-t border-gray-200">
                                  <td className="px-4 py-2">
                                    {item?.fecha_inicio_seg || "-"}
                                  </td>
                                  <td className="px-4 py-2">
                                    {item?.tiempo_transcurrido ==
                                    "La fecha es anterior a la fecha de la incidencia." ? (
                                  ...
                                  <td
                                    className="px-4 py-2 max-w-[600px]"
                                    title={
                      ...
                      <table className="min-w-full table-auto mb-5 border">
                        <thead>
                          <tr className="bg-gray-100">
                            ...
                            <th className="px-4 py-2 text-left border-b border-gray-300">
                              Monto Estimado de Daño ($)
                            </th>
                            <th className="px-4 py-2 text-left border-b border-gray-300">
                              Estatus
                            </th>
                            <th className="px-4 py-2 text-left border-b border-gray-300">
                              Duración Estimada Afectación
                            </th>
                            <th className="px-4 py-2 text-left border-b border-gray-300">
                              Evidencia
                            </th>
                        ...
                        <tbody>
                          {afectacionPatrimonial &&
                          afectacionPatrimonial.length > 0 ? (
                            afectacionPatrimonial.map(
                              (item: any, index: number) => (
                                <tr
                                  key={index}
                                  className="border-t border-gray-200">
                                  ...
                                  <td
                                    ...
                                    title={item?.descripcion_afectacion || "-"}>
                                    {" "}
            ...
            <div className="flex gap-2">
              <DialogClose asChild>
                <Button
                  className="w-full bg-gray-100 hover:bg-gray-200 text-gray-700 sm:w-2/3 md:w-1/2 lg:w-1/2 "
                  onClick={handleClose}>
                  Cancelar
                </Button>
              </DialogClose>
              ...
              <Button
                ...
                className="w-full bg-blue-500 hover:bg-blue-600 text-white sm:w-2/3 md:w-1/2 lg:w-1/2"
                disabled={isLoading}>
                {isLoading ? (
                  <>
